...
> 이용약관·개인정보처리방침·환불규정 페이지(`terms.html`/`privacy.html`/`refund.html`)는 만들어 뒀고, 푸터에 사업자정보가 표시된다. **사업자정보 값은 한 곳만 채우면** 모든 페이지 푸터에 반영됨.

- [ ] [shared/components/legal-footer.js](shared/components/legal-footer.js) 열어서 `BUSINESS_INFO`의 `[ ]` 자리 채우기:
  - `bizNo` = 사업자등록번호 (사업자등록증)
  - `mailOrderNo` = 통신판매업 신고번호 (정부24/공정위 조회 — 9번 참고)
  - `address` = 사업장 주소 (사업자등록증 소재지)
  - `email` = 고객문의 이메일 (스팸 대비 별도 주소 권장)
  - (상호 `citidel`, 대표 `윤태훈`은 이미 넣어둠 — 다르면 수정)
- [ ] `privacy.html`의 "개인정보 보호책임자 → 문의: [고객문의 이메일]"도 같은 값으로 수정
- [ ] (선택) 실제 상업 운영이면 약관·방침을 사업 형태에 맞게 법률 검토 권장 — 지금은 우리 서비스 사실관계(크레딧 모델·브라우저 처리·10일/500 환불)에 맞춰 작성해둠

> 안 채우면: 페이지는 정상 동작하고 `[사업자등록번호 …]` 같은 플레이스홀더가 그대로 보임. 라이브(실제 판매) 전엔 반드시 채울 것(전자상거래법 표시의무).
